...
if not df.empty:
    ...
    # 削除用チェックボックス列を付与したデータフレーム作成
    df_with_del = df.copy()
    df_with_del.insert(0, "選択", False)
    df_display = df_with_del.rename(columns={
    ...
    edited_df = st.data_editor(
        ...
        column_config={
            ...
            "給料(円)": st.column_config.NumberColumn(format="%d")
        },
        disabled=["日付", "出勤", "退勤", "労働(h)", "深夜(h)", "給料(円)"],
        hide_index=True,
        use_container_width=True,
        key="history_editor"
    )

    # チェックされた行のIDを取得して一括削除
    selected_ids = df[edited_df["選択"]]["id"].tolist()
    if selected_ids:
        if st.button(f"🗑️ 選択した {len(selected_ids)} 件を削除", type="primary", use_container_width=True):
            with conn.session as s:
                s.execute(text("DELETE FROM shifts WHERE id IN :ids"), {"ids": tuple(selected_ids)})
                s.commit()
            st.rerun()
else:
